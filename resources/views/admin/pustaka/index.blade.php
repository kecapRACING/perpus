...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('pustaka.create') }}" class="bg-blue-500 text-white px-6 py-3 rounded-lg shadow-md hover:bg-blue-600 transition duration-300">
        Tambah Pustaka Baru
    </a>
</div>
            ...
            <tr class="border-b hover:bg-gray-50">
                <td class="px-4 py-3">
                    ...
                    <img src="{{ asset('storage/' . $pustaka->gambar) }}"
                        class="mb-4 w-50 h-48 object-cover rounded-lg"
                ...
                <td class="px-4 py-3 flex items-center space-x-2">
                    <a href="{{ route('pustaka.edit', $pustaka->id_pustaka) }}" class="text-yellow-500 hover:text-yellow-700 transition duration-300">
                        Edit
                    </a>
                    <span>|</span>
                    <a href="{{ route('pustaka.show', $pustaka->id_pustaka) }}" class="text-blue-500 hover:text-blue-700 transition duration-300">
                        Lihat
                    </a>
                    <span>|</span>
                    <form action="{{ route('pustaka.destroy', $pustaka->id_pustaka) }}" method="POST" class="inline-block">
                        @csrf
                        @method('DELETE')
